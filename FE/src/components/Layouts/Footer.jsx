    ...
    const accordionData = [
        {
            title: 'Kategori',
            links: [
                { href: '#', text: 'Digital & Teknologi' },
                { href: '#', text: 'Pemasaran' },
                { href: '#', text: 'Manajemen Bisnis' },
                { href: '#', text: 'Pengembangan Diri' },
                { href: '#', text: 'Desain' },
            ]
        },
        {
            ...
            <footer className={`py-12 px-8 ${footerClasses}`}>
                <div className="container mx-auto">
                    ...
                    <div className="md:hidden text-center mb-10">
                        ...
                        {/* Menu Akordion */}
                        <div className={`w-full max-w-sm mx-auto border-t ${borderClasses}`}>
                            {accordionData.map(({ title, links }) => (
                                <div key={title} className={`border-b ${borderClasses}`}>
                                    <button onClick={() => setOpenAccordion(openAccordion === title ? null : title)} className="w-full flex justify-between items-center py-4 text-left font-semibold">
                                        <span>{title}</span>
                                        <svg className={`w-5 h-5 transform transition-transform ${openAccordion === title ? "rotate-180" : ""}`} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 9l-7 7-7-7"></path></svg>
                                    </button>
                                    <div className={`overflow-hidden transition-all duration-300 ease-in-out ${openAccordion === title ? 'max-h-screen' : 'max-h-0'}`}>
                                        <ul className="pb-4 space-y-3">
                                            {links.map(link => <li key={link.text}><a href={link.href} className={`text-sm ${linkClasses}`}>{link.text}</a></li>)}
                                        </ul>
                                    </div>
                                </div>
                            ))}
