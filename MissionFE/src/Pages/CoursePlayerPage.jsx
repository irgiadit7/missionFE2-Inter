                ...
                <div className="w-full md:w-1/4">
                    ...
                    <div className="grid grid-cols-5 gap-2">
                        {module.questions.map((q, index) => (
                            <button key={q.id} onClick={() => setCurrentQuestionIndex(index)} className={`flex items-center justify-center aspect-square rounded ${ currentQuestionIndex === index ? 'bg-green-600 text-white' : (userAnswers[q.id] ? (isDarkMode ? 'bg-gray-600' : 'bg-gray-300') : (isDarkMode ? 'bg-gray-700' : 'bg-gray-200')) }`}>
                                {index + 1}
                            </button>
                        ))}
                    </div>
                ...
                <div className="w-full md:w-3/4">
                    ...
                    <div className="space-y-3">
                        {currentQuestion.options.map((option) => (
                            <div key={option} onClick={() => handleAnswerSelect(currentQuestion.id, option)} className={`border p-4 rounded-lg cursor-pointer transition-colors flex items-center gap-4 ${ selectedAnswer === option ? `border-green-500 bg-green-50 dark:bg-green-900/20 ${isDarkMode ? 'text-white' : 'text-black'}` : `dark:border-gray-600 hover:bg-gray-100 dark:hover:bg-gray-800 ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}` }`}>
                    ...
                    <div className="mt-8 flex justify-between border-t dark:border-gray-700 pt-4">
                        ...
                        <button onClick={() => handleNavigateQuestion(1)} disabled={currentQuestionIndex === module.questions.length - 1} className="font-semibold text-green-600 hover:text-green-700 disabled:opacity-50">Selanjutnya &rarr;</button>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div className={`min-h-screen flex flex-col ${isDarkMode ? 'bg-gray-900 text-gray-300' : 'bg-gray-50 text-gray-700'}`}>
            ...
            <div className="container mx-auto flex flex-col md:flex-row flex-1">
                    ...
                    <div className="md:h-[calc(100vh-144px)] md:overflow-y-auto pr-2">
                        ...
                        <nav className="space-y-1">
                            {courseData.modules.map((module, index) => {
                                ...
                                const Icon = isCompleted ? CheckCircleIcon : module.type === 'video' ? PlayIcon : DocumentTextIcon;
                ...
                <main className="flex-1 p-6 md:p-10 pb-32 md:h-[calc(100vh-64px)] md:overflow-y-auto">
                    ...
                    {activeModule.type === 'congrats' && <CongratsContent module={activeModule} />}
                </main>
            </div>
            <footer className={`fixed bottom-0 left-0 right-0 z-10 border-t shadow-[0_-2px_5px_rgba(0,0,0,0.05)] ${isDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white'}`}>
                 <div className="container mx-auto flex justify-between items-center p-3">
                    ...
                    <button onClick={() => handleSelectModule(Math.min(courseData.modules.length - 1, activeModuleIndex + 1))} disabled={activeModuleIndex === courseData.modules.length - 1} className="flex items-center gap-2 px-4 py-2 bg-green-600 text-white rounded-lg font-semibold hover:bg-green-700 disabled:bg-gray-500 dark:disabled:bg-gray-600">Selanjutnya <ArrowRightIcon /></button>
                 </div>
            </footer>
        </div>
    );
