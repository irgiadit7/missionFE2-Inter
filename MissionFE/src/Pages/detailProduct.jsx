...
// Komponen Card Kursus - STYLING DIPERBARUI
const CourseCard = ({ course }) => (
    <Link to={`/products/${course.id}`} className="course-card bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden flex flex-col transition-all duration-300 hover:shadow-lg hover:-translate-y-1 group">
        <div className="w-full h-48 bg-white flex justify-center items-center p-4">
            <img src={course.image} alt={course.title} className="w-full h-full rounded-md object-cover" />
        </div>
        <div className="p-4 flex flex-col flex-grow">
            <h3 className="text-lg font-semibold mb-2 text-gray-800 group-hover:text-yellow-600 h-14 overflow-hidden">{course.title}</h3>
...
const DetailProductPage = () => {
    ...
    useEffect(() => {
        ...
        if (productData) {
            const priceInNumber = parseInt(productData.price.replace('Rp ', '').replace('k', ''));
            setProduct({ ...productData, price: priceInNumber, description: productData.desc });
        }
    }, [id]);
    ...
    return (
        <div className={isDarkMode ? 'bg-gray-800 text-white' : 'bg-[#FFFDF3] text-gray-800'}>
            <Header />
            <main className="container mx-auto px-4 py-8">
                {/* ... (Seluruh kode lainnya tetap sama) ... */}
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    <div className="lg:col-span-2">
                        ...
                        <div className="space-y-10">
                            <div><h2 className="text-2xl font-bold mb-3">Deskripsi</h2><p className="text-gray-600 dark:text-gray-500 leading-relaxed">{product.description}</p></div>
                            <div>
                                <h2 className="text-2xl font-bold mb-4">Belajar bersama Tutor Profesional</h2>
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    <div className={`p-6 border rounded-lg ${isDarkMode ? 'border-gray-700 bg-gray-900' : 'bg-white'}`}><div className="flex items-center gap-4"><img src="/images/avatar.png" alt={product.author} className="w-12 h-12 rounded-full" /><div><h3 className="font-bold text-lg">{product.author}</h3><p className="text-sm text-gray-500">Tutor di bidang {product.category}</p></div></div><p className="text-sm text-gray-600 dark:text-gray-400 mt-4">Berpengalaman lebih dari 5 tahun di industri dan telah membantu ratusan siswa mencapai impian karir mereka.</p></div>
                                    <div className={`p-6 border rounded-lg ${isDarkMode ? 'border-gray-700 bg-gray-900' : 'bg-white'}`}><div className="flex items-center gap-4"><img src="/images/avatar.png" alt="Jane Doe" className="w-12 h-12 rounded-full" /><div><h3 className="font-bold text-lg">Jane Doe</h3><p className="text-sm text-gray-500">Asisten Tutor</p></div></div><p className="text-sm text-gray-600 dark:text-gray-400 mt-4">Spesialis dalam memberikan feedback dan membantu siswa mengatasi kesulitan belajar.</p></div>
                                </div>
                            </div>
                            <div><h2 className="text-2xl font-bold mb-3">Kamu akan Mempelajari</h2><div className={`rounded-lg border p-4 ${isDarkMode ? 'border-gray-700 bg-gray-900' : 'bg-white'}`}>{courseMaterials.map((item, index) => (<AccordionItem key={index} title={item.title} isOpen={openAccordion === index} onClick={() => setOpenAccordion(openAccordion === index ? null : index)}><div className="space-y-4">{item.videos.length > 0 ? (item.videos.map((video, videoIndex) => (<div key={videoIndex} className="flex justify-between items-center text-gray-600 dark:text-gray-400"><div className="flex items-center"><svg className="w-6 h-6 mr-3 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M14.752 11.168l-3.197-2.132A1 1 0 0010 9.87v4.263a1 1 0 001.555.832l3.197-2.132a1 1 0 000-1.664z"></path><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg><span className="font-medium">{video.title}</span></div><span className="text-sm font-medium">{video.duration}</span></div>))) : (<p className="text-sm text-gray-400">Detail untuk bab ini akan segera tersedia.</p>)}</div></AccordionItem>))}</div></div>
                            <div><h2 className="text-2xl font-bold mb-4">Rating dan Review</h2><div className={`p-6 border rounded-lg shadow-md ${isDarkMode ? 'border-gray-700 bg-gray-900' : 'bg-white'}`}><div className="grid grid-cols-1 md:grid-cols-2 gap-8">{reviews.map((review, index) => (<div key={index} ><div className="flex items-center gap-4 mb-3"><img src={review.avatar} alt={review.name} className="w-12 h-12 rounded-full" /><div><h3 className="font-bold text-lg">{review.name}</h3><p className="text-sm text-gray-500">{review.batch}</p></div></div><p className="text-gray-600 dark:text-gray-400 my-4 text-sm">"{review.reviewText}"</p><div className="flex items-center gap-2"><StarRating rating={review.rating} /><span className="text-sm font-bold">{review.rating}</span></div></div>))}</div></div></div>
                        </div>
                    </div>
                    <div className="lg:col-span-1">
                         <div className="sticky top-24">
                            <div className={`border rounded-lg shadow-lg p-6 ${isDarkMode ? 'border-gray-700 bg-gray-900' : 'bg-white'}`}><h2 className="font-bold text-xl mb-4">Gapai Karier Impianmu</h2><p className="text-3xl font-bold text-green-600 mb-4">{`Rp ${product.price}k`}</p><div className="space-y-3"><button className="w-full bg-green-500 text-white px-8 py-3 rounded-lg font-semibold hover:bg-green-600 transition-colors text-lg">Pesan Sekarang</button><button className="w-full bg-transparent border border-green-500 text-green-500 px-8 py-3 rounded-lg font-semibold hover:bg-green-500 hover:text-white transition-colors text-lg">Beli Langsung</button></div><div className="mt-6 space-y-3 text-sm text-gray-600 dark:text-gray-400"><p><span className="font-bold">Termasuk:</span></p><ul className="list-disc list-inside"><li>12 Video Pembelajaran</li><li>7 Latihan Praktis</li><li>Sertifikat Penyelesaian</li><li>Akses Selamanya</li><li>Grup Komunitas</li></ul></div></div>
                        </div>
                    </div>
                </div>
                <div className="mt-16">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
                        {relatedProducts.length > 0 ? (relatedProducts.map(course => (<CourseCard key={course.id} course={course} />))) : (<p className="text-center col-span-3 text-gray-500">Tidak ada kursus terkait lainnya di kategori ini.</p>)}
                    </div>
                </div>
            </main>
            <Footer />
