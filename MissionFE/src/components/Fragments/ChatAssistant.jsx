...
const ChatAssistant = ({ isChatOpen, handleToggleChat }) => { 
    ...
    return (
        <>
            ...
            {/* Gunakan isChatOpen untuk menampilkan/menyembunyikan jendela chat */}
            {isChatOpen && (
                <div className={`fixed bottom-24 right-6 w-80 h-[450px] rounded-xl shadow-2xl flex flex-col z-50 transition-all duration-300 ${isDarkMode ? 'bg-[#0b141a]' : 'bg-[#E0E0E0]'}`}>
                    <div className="bg-green-600 text-white p-3 flex justify-between items-center rounded-t-xl">
                        ...
                        <button onClick={handleToggleChat} className="hover:opacity-75"> {/* Tombol close juga pakai fungsi dari props */}
                    ...
                    <div ref={chatBodyRef} className="flex-1 p-3 overflow-y-auto chat-body-scrollbar">
                        {messages.map((msg, index) => (
                            <div key={index} className={`mb-2 flex ${msg.from === 'bot' ? 'justify-start' : 'justify-end'}`}>
                                <div className={`relative py-2 px-3 rounded-lg max-w-[85%] shadow-sm ${msg.from === 'bot' ? (isDarkMode ? 'bg-[#2a3942]' : 'bg-white') : (isDarkMode ? 'bg-[#005c4b]' : 'bg-[#dcf8c6]')}`}>
                                ...
                                <div className="flex flex-col items-start space-y-1">
                                    {quickReplies.map((reply) => (
                                        <button
                                            ...
                                            className={`w-full text-left text-sm p-2 rounded transition-colors ${isDarkMode ? 'text-blue-300 hover:bg-gray-700' : 'text-blue-600 hover:bg-gray-100'}`}
                                        >
                                            {reply.label}
                                        </button>
                                    ))}
                    ...
                    <div className={`p-2 flex items-center ${isDarkMode ? 'bg-transparent' : 'bg-transparent'}`}>
                        <input
                            ...
                            disabled={isLoading}
                            className={`flex-1 p-2 border-none rounded-full focus:outline-none disabled:opacity-50 ${isDarkMode ? 'bg-[#2a3942] text-white' : 'bg-white text-black'}`}
                        />
